...
The web interface for Postgres1st WatchTower. It is a React application served through Grafana by a
companion plugin.

See the [repository root README](../README.md) for what Postgres1st WatchTower is and how it is installed.
...
Use this when you want to drive Vite from your IDE without `make env`. You still need a reachable server — the simplest way is to leave the devcontainer running (`make env-up`) so its ports are exposed; any other Postgres1st WatchTower server reachable at `https://localhost:8443` works too.
