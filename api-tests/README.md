...
API tests for Postgres1st WatchTower 3.x
...
1. Run a Postgres1st WatchTower server. This can be done by running `make env-up` in the repository root.
...
where `PMM_SERVER_URL` should be pointing to a running Postgres1st WatchTower server. The variable keeps
its `PMM_` name because the Makefile guards on it (`guard-PMM_SERVER_URL`).

If the server is located locally:
