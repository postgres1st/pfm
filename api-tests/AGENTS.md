# PFMM API Tests Development Guidelines
...
The `/api-tests` directory contains **integration tests** for PFMM APIs. These tests run against a **live PFMM Server** and verify end-to-end API behavior using the generated Go HTTP clients from Swagger specs.
...
Go test binary
  → Generated Swagger HTTP clients (from /api/*/json/client/)
    → HTTP/JSON requests to PFMM Server (gRPC-Gateway)
...
### Prerequisites
- Running PFMM Server (use `make env-up` from repo root)
...
- Don't hardcode the PFMM Server URL — use the `-pmm.server-url` flag
