...
func TestPackages(t *testing.T) {
	cmd := exec.Command("pfw-managed", "-h")
...
func TestImports(t *testing.T) {
	...
	for _, service := range []string{
		...
		"github.com/percona/pmm/managed/cmd/pfw-managed-init",
		"github.com/percona/pmm/managed/cmd/pfw-managed-starlark",
...
// The defect: kingpin's HasEnvarValue() is os.Getenv(...) != "", so an env var set
// to the empty string is indistinguishable from an absent one and resolves to
// Default("pmm-managed"). initWithRoot then ALTERs the role to that constant, which
// is exactly the rotation-back-to-the-shipped-value this branch exists to prevent.
func TestResolveManagedDBPassword(t *testing.T) {
	t.Parallel()

	t.Run("set-but-empty resolves from the secret store", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, "generated",
			resolveManagedDBPassword("pmm-managed", true, "", "generated"))
	})

	t.Run("set-but-empty with no store keeps the default", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, "pmm-managed",
			resolveManagedDBPassword("pmm-managed", true, "", ""))
	})

	// The container: no envar, no store, the constant is genuinely correct.
	t.Run("unset envar keeps kingpin's answer", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, "pmm-managed",
			resolveManagedDBPassword("pmm-managed", false, "", "generated"))
	})

	t.Run("a non-empty envar is already kingpin's answer", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, "fromenv",
			resolveManagedDBPassword("fromenv", true, "fromenv", "generated"))
	})

	// --postgres-password on the command line beats the envar in kingpin, so an
	// explicit value must survive even when the envar is empty and a store exists.
	t.Run("an explicit flag value is not overridden", func(t *testing.T) {
		t.Parallel()
		assert.Equal(t, "explicit",
			resolveManagedDBPassword("explicit", true, "", "generated"))
		assert.Equal(t, "explicit",
			resolveManagedDBPassword("explicit", false, "", "generated"))
	})
}
