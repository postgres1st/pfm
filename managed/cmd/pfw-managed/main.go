...
const (
	...
	distributionInfoFilePath = "/srv/pfw-distribution"
	// Pre-rebrand path. Still written by the Docker entrypoint, which is not rebranded,
	// and present on any host provisioned before the rename.
	legacyDistributionInfoFilePath = "/srv/pmm-distribution"
...
// resolveManagedDBPassword picks the pmm-managed role password kingpin should have
// produced. Exactly one case differs from kingpin's own answer: the envar is SET to
// the empty string, which kingpin treats as absent (HasEnvarValue is os.Getenv != "")
// and silently replaces with the flag's Default. An empty value there means the secret
// file was truncated or unreadable, never that the shipped constant is wanted -- so
// read the store instead.
//
// The flagValue == dbsecret.Legacy guard keeps an explicit --postgres-password
// intact: kingpin lets the command line beat the envar, so without it a deliberate
// value would be overwritten by the store whenever the envar happened to be empty.
func resolveManagedDBPassword(flagValue string, envSet bool, envValue, storeValue string) string {
	if envSet && envValue == "" && flagValue == dbsecret.Legacy && storeValue != "" {
		return storeValue
	}
	return flagValue
}
...
func main() { //nolint:gocognit,maintidx,cyclop
	...
	clickhousePasswordF := kingpin.Flag("clickhouse-password", "Clickhouse database user password").Default(dbsecret.ClickhousePassword()).Envar("PMM_CLICKHOUSE_PASSWORD").String()
	...
	// kingpin decides an envar is present with HasEnvarValue(), which is
	// os.Getenv(...) != "". So PMM_POSTGRES_DBPASSWORD set to the EMPTY string --
	// a truncated or half-written /srv/.pfw-secrets/managed-db.env, an
	// EnvironmentFile systemd could not read -- is treated as absent and falls
	// through to Default("pmm-managed"). That is not merely a failed connection:
	// initWithRoot runs `ALTER USER ... WITH PASSWORD` with whatever we hold
	// (managed/models/database.go), so an empty env value would rotate a generated
	// credential back to the shipped constant and publish it. Resolve from the
	// secret store instead. The Default stays because the container image really
	...
	*postgresDBPasswordF = resolveManagedDBPassword(
	...
	if *haEnabled {
		models.AgentConfigFilePath = "/srv/pfw-agent/config/pfw-agent.yaml"
	}
	...
	// Fall back to the pre-rebrand path: the Docker entrypoint still writes it, and a
	// host provisioned before the rename has only that file. Reading a missing marker is
	// not fatal here -- it degrades to an unknown distribution -- but reporting "unknown"
	// for a host that plainly told us what it is would be a silent regression.
	distPath := distributionInfoFilePath
	if _, err := os.Stat(distPath); os.IsNotExist(err) {
		if _, lerr := os.Stat(legacyDistributionInfoFilePath); lerr == nil {
			distPath = legacyDistributionInfoFilePath
		}
	}
	dus := distribution.NewService(distPath, osInfoFilePath, l)
