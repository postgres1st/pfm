...
**pmm-agent** is the client-side monitoring agent for PGF WatchTower. It runs on every monitored host, managing metric exporters as child processes, running built-in QAN and Real-Time Analytics (RTA) collectors in-process, executing on-demand actions (explain, PT summary), and performing backup/restore jobs. It communicates with pmm-managed on the PGF WatchTower Server over a persistent bidirectional gRPC stream.
...
### Communication with PGF WatchTower Server
...
- **Key settings**:
  - `server.address`, `server.username`, `server.password` — PGF WatchTower Server connection
