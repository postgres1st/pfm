...
**pmm-agent** is the client-side monitoring agent for Postgres1st WatchTower. It runs on every monitored host, managing metric exporters as child processes, running built-in QAN and Real-Time Analytics (RTA) collectors in-process, executing on-demand actions (explain, PT summary), and performing backup/restore jobs. It communicates with pmm-managed on the Postgres1st WatchTower Server over a persistent bidirectional gRPC stream.
...
### Communication with Postgres1st WatchTower Server
...
- **Sources**: YAML file (`pmm-agent.yaml`), CLI flags, environment variables (`PFW_AGENT_*`)
- **Runtime access**: `config.Storage` provides thread-safe `Get()` and `Reload()`
- **Key settings**:
  - `server.address`, `server.username`, `server.password` — Postgres1st WatchTower Server connection
