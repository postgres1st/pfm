...
- a current backup of your Postgres1st WatchTower data volume
...
1. Create a backup snapshot of your Postgres1st WatchTower data volume:
...
1. Go to **All dashboards > Postgres1st WatchTower Health > Postgres1st WatchTower Health** and check that all services are running. 
...
1. Stop the new Postgres1st WatchTower container:
   ```bash
   systemctl --user stop pmm-server
   ```

2. Restore using your pre-upgrade snapshot. Create a volume from the snapshot, attach it to the instance, and start the previous Postgres1st WatchTower version.
