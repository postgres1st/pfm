...
- a current backup of your PGF WatchTower data volume
...
1. Create a backup snapshot of your PGF WatchTower data volume:
...
1. Go to **All dashboards > PGF WatchTower Health > PGF WatchTower Health** and check that all services are running. 
...
1. Stop the new PGF WatchTower container:
   ```bash
   systemctl --user stop pmm-server
   ```

2. Restore using your pre-upgrade snapshot. Create a volume from the snapshot, attach it to the instance, and start the previous PGF WatchTower version.
