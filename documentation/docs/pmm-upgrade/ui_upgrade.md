...
To use the UI upgrade feature, you must have the Watchtower updater installed and properly configured with your PMM Server.

If the Watchtower updater is not installed, the UI upgrade options will not be available. See [Running PMM Server with the Watchtower updater](../install-pmm/install-pmm-server/deployment-options/docker/index.md) for setup instructions.

!!! note
    Since the Watchtower updater is no longer actively maintained, new installations should use CLI or container-based upgrade methods instead.
