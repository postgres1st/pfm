...
1. If you are switching from [UI-based upgrades](ui_upgrade.md), stop and remove the Watchtower updater container before moving to manual Podman upgrades. UI-based upgrades used the Watchtower updater to apply Postgres1st WatchTower images automatically. If the Watchtower updater is still running, it may update Postgres1st WatchTower during your manual upgrade and cause conflicts.
...
1. Update Postgres1st WatchTower version by editing the PMM Server environment file. Replace 3.0.0 with your target version number:
