# About Postgres1st WatchTower Inventory

The **Inventory** dashboard is a high-level overview of all objects registered in Postgres1st WatchTower.
...
- Each instance of a service gets a `version` attribute to the response of the endpoint that provides a list of services being monitored by Postgres1st WatchTower. This makes it easy to visualize the database server version.

    Limitations:

    - The version is not captured for the internal PostgreSQL database.
    - The version is only captured when a new service is being added to Postgres1st WatchTower and the agent installed on the client side is equal to or greater than v2.41.0.
...
- `pfw-agent` is at the top of the tree, assigned to pfw-agent itself
...
![!Postgres1st WatchTower Inventory Service Agent Properties](../images/PMM_Inventory_Service_Agent_Properties.png)

#### Agent status indicators

Agents in Postgres1st WatchTower Inventory display the following status indicators:
...
- **Deployment**: Services within Postgres1st WatchTower are deployed on nodes and rely on them for resources, such as CPU, memory, and storage, to execute tasks.
...
3. Click **Edit** to change the labels, then click **Save Changes**. 
    ...
    ![Postgres1st WatchTower - editing labels](../images/PMM_access_edit_labels.png)
...
Editing existing labels can impact the following Postgres1st WatchTower functions:
