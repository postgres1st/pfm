# About PGF WatchTower Inventory

The **Inventory** dashboard is a high-level overview of all objects registered in PGF WatchTower.
...
- Each instance of a service gets a `version` attribute to the response of the endpoint that provides a list of services being monitored by PGF WatchTower. This makes it easy to visualize the database server version.

    Limitations:

    - The version is not captured for the internal PostgreSQL database.
    - The version is only captured when a new service is being added to PGF WatchTower and the agent installed on the client side is equal to or greater than v2.41.0.
...
![!PGF WatchTower Inventory Service Agent Properties](../images/PMM_Inventory_Service_Agent_Properties.png)

#### Agent status indicators

Agents in PGF WatchTower Inventory display the following status indicators:
...
- **Deployment**: Services within PGF WatchTower are deployed on nodes and rely on them for resources, such as CPU, memory, and storage, to execute tasks.
...
3. Click **Edit** to change the labels, then click **Save Changes**. 
    ...
    ![PGF WatchTower - editing labels](../images/PMM_access_edit_labels.png)
...
Editing existing labels can impact the following PGF WatchTower functions:
