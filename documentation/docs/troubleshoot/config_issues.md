...
PGF WatchTower can also generate diagnostics data that can be examined and/or shared with our support team to help solve an issue. You can get collected logs from PMM Client using the pfw-admin summary command.
...
1. From the main menu, choose **Help > PGF WatchTower Logs > Export logs**.
...
When adding clients to the PMM Server, you use the `admin` user. However, if you change the password for the admin user from the PGF WatchTower UI, then the clients will not be able to access PGF WatchTower due to authentication issues. Also, Grafana will lock out the admin user due to multiple unsuccessful login attempts.
...
After restarting Docker, PGF WatchTower dashboard returns *500 Internal Server Error*.
