# PFMM data encryption

Postgres1st (PFMM) implements robust encryption for sensitive data stored in its internal database's `agent` table. This includes access credentials and configuration details.

## Default encryption

PFMM automatically manages encryption using a key file located at `/srv/pmm-encryption.key`. PFMM generates this file upon the initial launch of PFMM 3 or when upgrading from the latest version of PFMM 2.

## Custom encryption key configuration

For enhanced security control, PFMM supports custom encryption keys.
...
PFMM uses this key with the TINK `AES256GCMKeyTemplate` output prefix type.
...
!!! hint alert alert-success "Important"
    Configure this **before** any data encryption occurs: either before upgrading to PFMM 3 or before initially starting a new PFMM 3.x instance.

### Key management requirements

Once configured, PFMM will use the custom key to encrypt and decrypt all sensitive data stored within the system.

If the custom key is unavailable or misplaced, PFMM will be unable to access and decrypt the stored data, which will prevent it from running correctly.

Make sure to store and manage the custom encryption key securely to avoid potential loss of data access.

## Rotating the encryption key

You may want to generate a new encryption key or rotate it when the original key is compromised or as part of routine security maintenance. For this, you can use the **PFMM Encryption Rotation Tool**.
...
2. Run the Encryption Rotation Tool using the following command:
    ...
    - If using custom credentials/SSL for the PFMM internal database, provide them with the appropriate flags.

3. Verify PFMM functionality all components are functioning properly to ensure that the encryption key rotation was successful.
...
- Always keep a secure backup of your encryption key, especially when using `PMM_ENCRYPTION_KEY_PATH`, as it is critical to PFMM’s data decryption process.
