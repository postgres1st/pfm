...
You can manage roles in Postgres1st WatchTower by editing or deleting a role.
...
When a user signs in to Postgres1st WatchTower for the first time and the user has no role assigned, the user is automatically assigned the **Default** role. For administrators, the default role provides a convenient way to configure default permissions for new users.
