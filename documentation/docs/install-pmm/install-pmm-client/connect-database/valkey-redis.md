# Connect Valkey and Redis databases to Postgres1st WatchTower

Valkey is a high-performance open-source alternative to Redis. Because Valkey is a Redis fork that maintains full protocol compatibility, Postgres1st WatchTower monitors both databases using the same proven methods and dashboards.

Connect your Valkey or Redis instances to Postgres1st WatchTower to track performance, analyze commands, and monitor cluster health. Postgres1st WatchTower's 10 dedicated **Valkey/Redis** dashboards help you spot memory issues, diagnose slow queries, and maintain healthy replication across your Valkey/Redis deployments.

### Prerequisites

Before connecting Valkey or Redis to Postgres1st WatchTower, review the prerequisites for your monitoring setup:
...
You can add your Valkey or Redis service to Postgres1st WatchTower either through the user interface or via the command line:
...
=== ":material-web: Via UI"
    ...
    3. Fill in the **Main details** section:
        ...
        - **Nodes**: Select the Postgres1st WatchTower node where the agent is running.
        ...
        - **Connection timeout**: How long Postgres1st WatchTower should wait when connecting to this service. Increase this for remote or high-latency databases. If the connection times out, Postgres1st WatchTower retries the next time it collects metrics. Leave empty to use the default of 3s.
...
=== ":material-cog: Via inventory commands (Advanced)"
    Postgres1st WatchTower also provides inventory commands for more granular control:

    === ":material-database-plus: Add Valkey service via inventory"
        ```sh
        pfw-admin inventory add service valkey \
    ...
    === ":material-robot: Add Valkey exporter agent"
        ```sh
        pfw-admin inventory add agent valkey_exporter \
        --address=localhost:6379 \
        --username=pmm \
        --password=StrongPassword123!
        ```
...
If the service is added successfully, Postgres1st WatchTower displays:
```sh
...
After adding your Valkey or Redis service to Postgres1st WatchTower, verify that it's properly connected and collecting data.
...
=== ":material-console: Via command line"
    ...
    1. List all Valkey/Redis services and their status:
        ...
        ```bash
        pfw-admin inventory list services --service-type=valkey
        ```

    2. List all Valkey/Redis exporter agents:

        ```bash
        pfw-admin inventory list agents --agent-type=valkey_exporter
        ```
...
2. Navigate to the **Valkey/Redis** dashboards from the left menu. Postgres1st WatchTower provides 10 dedicated dashboards:
...
=== ":material-console: Via command line"
    ...
    ```bash
    pfw-admin remove valkey Valkey-Primary
    ```

    Or use inventory commands:
    ```bash
    # Remove service
    pfw-admin inventory remove service <service-id>
...
After successfully connecting your Valkey or Redis instance to Postgres1st WatchTower:
...
- Use [Postgres1st WatchTower Inventory](../../../use/dashboard-inventory.md) to view and manage all monitored instances.
- [Valkey official documentation](https://valkey.io/docs/)
- [Redis official Documentation](https://redis.io/docs/)
- [pfw-admin command reference](../../../use/commands/pmm-admin/pmm-admin.md)
