...
=== "With root permissions"
    ...
    5. Set the installation directory:
        ...
        ```sh
        export PMM_DIR=/opt/postgres1st/watchtower
        ```
    ...
    8. Create symbolic links to make Postgres1st WatchTower commands available system-wide:
        ...
        sudo ln -s /opt/postgres1st/watchtower/bin/pfw-admin /usr/local/bin/pfw-admin
        ```
    9. Set up the agent:

        ```sh
        sudo pfw-agent setup --config-file=/opt/postgres1st/watchtower/config/pfw-agent.yaml --server-address=192.168.1.123 --server-insecure-tls --server-username=admin --server-password=admin
        ```

    10. Run the agent:

        ```sh
        sudo pfw-agent --config-file=${PMM_DIR}/config/pfw-agent.yaml
        ```
...
=== "Without root permissions"
    ...
    8. Set up the agent:
        ...
        ```sh
        pfw-agent setup --config-file=${PMM_DIR}/config/pfw-agent.yaml --server-address=192.168.1.123 --server-insecure-tls --server-username=admin --server-password=admin --paths-tempdir=${PMM_DIR}/tmp --paths-base=${PMM_DIR}
        ```

    9. Run the agent:

        ```sh
        pfw-agent --config-file=${PMM_DIR}/config/pfw-agent.yaml
        ```
...
=== "Using Service accounts (Recommended)"
    ...
    11. Register using the token:
        ...
        ```bash
        pfw-admin config --server-insecure-tls \
