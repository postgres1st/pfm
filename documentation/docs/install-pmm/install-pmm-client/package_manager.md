...
Postgres1st (PGF WatchTower) Client can be installed using standard Linux package managers. You can choose between automated repository setup or manual package download options.
...
=== "Manual download"
    ...
    1. Visit the [PGF WatchTower download page](https://www.percona.com/downloads/).
    2. Select PGF WatchTower 3 and choose specific version (usually the latest).
...
=== "Using Service accounts (Recommended)"
    ...
    4. Enter a descriptive name (e.g.: `pmm-client-prod-db01`). PGF WatchTower automatically shortens names exceeding 200 characters using a `{prefix}_{hash}` pattern.
