# Postgres1st WatchTower installation overview

Installing Postgres1st (Postgres1st WatchTower) involves setting up a central PMM Server and distributed PMM Clients that work together to monitor your database environment. 
...
The Postgres1st WatchTower installation consists of three main steps that need to be completed in sequence: 
{.power-number}
...
3. [Configure monitoring services](#3-add-services-for-monitoring): connect Postgres1st WatchTower to your database instances, select which metrics to collect, and customize monitoring parameters

## Plan the installation

Before ou install Postgres1st WatchTower, ensure your environment is properly prepared:
...
| [Virtual Appliance](../install-pmm/install-pmm-server/deployment-options/virtual/index.md) | Traditional VM environments | • Pre-configured virtual machine<br>• Works with VirtualBox<br>• No container knowledge required<br>• Isolated environment | • Larger resource footprint<br>• VM management overhead<br>• Less flexible than containers |
...
Install and run at least one PMM Server using one of the following deployment methods. If you're not sure which deployment method is best for your environment, check out this [Choose a Postgres1st WatchTower deployment strategy](../install-pmm/plan-pmm-installation/choose-deployment.md) topic for a comparison of your options.
...
Postgres1st WatchTower supports monitoring across the following database technologies, cloud services, proxy services, and system metrics:
