...
!!! danger alert alert-danger "Not recommended for production environments"
    Using a host directory for Postgres1st WatchTower data persistence is not recommended for production environments. This approach may lead to permission issues, inconsistent backup behavior, and potential data corruption during upgrades. 
...
- when you need direct filesystem access to Postgres1st WatchTower data
...
2. Create and identify a directory on the host where to store Postgres1st WatchTower data. For example, `/home/user/srv`.
...
5. Access the Postgres1st WatchTower web interface at `https://localhost:443` in a web browser. If you're connecting from a different machine, replace `localhost` with your server's IP address or hostname.
