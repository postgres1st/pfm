...
### Feature controls
Enable or disable specific Postgres1st WatchTower features:
...
Control Nomad client-side garbage collection (GC) directly from Postgres1st WatchTower using environment variables. 
...
Postgres1st WatchTower includes experimental environment variables prefixed with `PERCONA_TEST_*` that are under development and subject to change. To see the complete list and details of experimental variables, see [Preview environment variables](preview_env_var.md).
...
### Variables for migrating from Postgres1st WatchTower v2 to Postgres1st WatchTower v3

Postgres1st WatchTower v3 introduces several important changes to improve consistency and clarity. When migrating from Postgres1st WatchTower v2 to Postgres1st WatchTower v3, you'll need to update your environment variables to match the new naming convention: 
...
To see the full lists of variable name changes between Postgres1st WatchTower v2 and Postgres1st WatchTower v3, see the [Migration guide](../../../../pmm-upgrade/migrating_from_pmm_2.md#variables-for-migrating-from-pmm-v2-to-pmm-v3).
...
docker run \
 ...
 -e PMM_DEBUG=true \
 -e PMM_ENABLE_ACCESS_CONTROL=true \
 percona/pmm-server:3
``` 
