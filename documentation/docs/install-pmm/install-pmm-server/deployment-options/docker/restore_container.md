...
You can restore PMM Server either from a manual backup or from an automated backup volume that was created during migration to PGF WatchTower v3.
...
- an [automated backup volume](../../../../pmm-upgrade/migrating_from_pmm_2.md#step-3-migrate-to-the-latest-pmm-3) created during migration from PGF WatchTower V3
...
=== "Directory backup"
    ...
    3. Copy backup data to PGF WatchTower volume:
        ```sh
        # Remove current volume (WARNING: This deletes current data)
        docker volume rm pmm-data
...
=== "Migration rollback"
    Rollback from PGF WatchTower 3 to PGF WatchTower 2 using automated migration backup:
    {.power-number}

    1. Stop the current PGF WatchTower v3 container:
        ```sh
        docker stop pmm-server
        ```

    2. Remove the PGF WatchTower v3 container:
        ```sh
        docker rm pmm-server
        ```

    3. Start a PGF WatchTower v2 container using your backup volume:
        ```sh
        docker run -d \
        -p 443:443 \
        --volume <backup-volume-name>:/srv \
        ...
        Replace `<backup-volume-name>` with your PGF WatchTower v2 backup volume name (e.g., `pmm-data-2025-01-16-165135`).

    4. Verify that your PGF WatchTower v2 instance is running correctly:
        ```sh
        docker logs pmm-server
...
=== "Universal container restore"
    ...
    - you have a backup directory with an `srv/` folder containing PGF WatchTower data
    ...
    5. Copy the backup data to the PGF WatchTower data volume:
        ```sh
...
- [Migrate from PGF WatchTower v2 to v3](../../../../pmm-upgrade/migrating_from_pmm_2.md) if restoring to upgrade