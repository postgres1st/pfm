...
=== "Let's Encrypt certificate"
    ...
    # Stop Postgres1st WatchTower temporarily
    systemctl --user stop pmm-server

    # Configure Postgres1st WatchTower to use the certificate
...
sudo ufw allow 443/tcp   # HTTPS Postgres1st WatchTower interface
sudo ufw --force enable
```
## Manage users and access

After the initial setup, create additional user accounts in Postgres1st WatchTower for your team members. Follow the principle of least privilege when assigning user roles.
{.power-number}
...
### Resize Postgres1st WatchTower data volume
...
2. Expand the file system to use the additional space:
    ...
    ```sh
    # SSH to your Postgres1st WatchTower instance
...
3. Postgres1st WatchTower automatically detects the storage increase within ~5 minutes and adjusts its configuration.
...
2. Expand the disk from AWS Console/CLI to the desired capacity:
    ...
    - Log in to the Postgres1st WatchTower EC2 instance and verify that the disk capacity has increased. For example, if you have expanded disk from 8G to 10G, `dmesg`:
...
Postgres1st WatchTower fully supports resizing EC2 instances, as long as you follow the steps outlined in the [AWS EC2 resizing guide](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-resize.html).


!!! note "Data safety"
    Postgres1st WatchTower uses a separate EBS volume for monitoring data, so changing instance types doesn't affect your collected metrics or dashboards.
...
pfw-admin config --server-insecure-tls --server-url=https://admin:your-password@<pmm-server-ip>:443
```
...
1. Create a new volume using the latest snapshot of the Postgres1st WatchTower data volume:

    ![Create Volume](../../../../images/aws-marketplace.pmm.ec2.backup2.png)

2. Stop the PMM Server instance.

3. Detach the current Postgres1st WatchTower data volume:
...
=== "From AWS CLI"
    ...
    2. Disconnect all PMM clients:
       ...
       pfw-admin remove --all
       ```

    3. Export configuration:
       ```bash
       podman  exec pmm-server pfw-admin summary > pmm-final-config.txt
       ```

    4. Stop Postgres1st WatchTower services:
       ```bash
