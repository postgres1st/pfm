...
3. Select the latest version (recommended), choose the AWS region where you want to deploy PFMM, then click **Continue to Launch**.
...
2. Locate your new PFMM instance in the EC2 instances table. Initially, its Status shows **Pending** while launching and the Name will be empty. 
...
2. Open the IP address in a web browser and log into PFMM using the default credentials:
    - **Username:** `admin` 
    - **Password:** Your EC2 instance ID  available in the **Instance ID** field in the EC2 console.
    
    ![PFMM Login](../../../../images/PMM_Login.png)
