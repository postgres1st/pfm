...
Postgres1st WatchTower High Availability Cluster uses a two-step installation process that separates database operators from monitoring components. This separation simplifies upgrades and prevents cleanup issues when uninstalling.
...
#### Step 2: Install Postgres1st WatchTower HA

Installs monitoring infrastructure:

- **3 Postgres1st WatchTower monitoring servers** with automatic leader election using Raft consensus (one active leader, two standbys)
...
To install Postgres1st WatchTower HA:

=== "Quickstart installation"

    Get Postgres1st WatchTower HA Cluster running in 10 minutes with this simplified setup. For advanced configuration options, use the full installation option.
    {.power-number}
    ...
    4. Create Postgres1st WatchTower secret with your passwords:
      ```sh
    ...
    5. Install Postgres1st WatchTower HA:
      ```sh
...
=== "Full installation"
    ...
    Postgres1st WatchTower needs three operators to run on Kubernetes. You can install all of them with one command, or install them separately if you need custom configurations:
    {.power-number}
    ...
    ### Step 4: Create Postgres1st WatchTower credentials secret

    The `secret.create` parameter is set to `false` by default in the Helm chart. You must create the `pmm-secret` manually before installing Postgres1st WatchTower HA.
    ...
    ### Step 5: Install Postgres1st WatchTower HA
    ...
    === "Custom configuration"
        ...
        This approach gives you full control over your Postgres1st WatchTower HA deployment settings.
        {.power-number}
    ...
    ### Step 6: Verify installation
      ...
      ```sh
        ...
        # Wait for all Postgres1st WatchTower pods to be ready
        kubectl wait --for=condition=ready pod \
          -l app.kubernetes.io/name=pmm \
          -n pmm --timeout=600s
      ```

## Access Postgres1st WatchTower after installation
...
Postgres1st WatchTower HA exposes multiple service endpoints for different purposes. For all external connections to Postgres1st WatchTower (including PMM Clients, web browsers, API calls, and Percona Operators) always use **`pmm-ha-haproxy`**. 

This load balancer automatically routes traffic to the active Postgres1st WatchTower leader and handles failover transparently.
...
| `monitoring-service` | Headless service for direct Postgres1st WatchTower pod access. **Do not use for external connections** as it bypasses HAProxy and can cause failures during leader changes | 8443 (HTTPS) | Internal cluster communication only |

#### Access database components (advanced)

For direct database access or troubleshooting, Postgres1st WatchTower HA also exposes:
...
## Configure Postgres1st WatchTower HA
...
=== "ClusterIP (Default)"
    ...
    **Best for**: Internal cluster access only (recommended for security)
    ...
    ClusterIP makes Postgres1st WatchTower accessible only within the Kubernetes cluster. This is the default setting and requires no configuration changes.
    ...
    **Option 2: Port-forward for local testing**
    ...
    Access Postgres1st WatchTower from your local machine for testing or administration:
    {.power-number}
    ...
    3. Leave the port-forward running while you access Postgres1st WatchTower. Press `Ctrl+C` to stop.
...
=== "Amazon EKS"
    ...
    **Best for**: AWS deployments requiring external access
    ...
    !!! tip "AWS best practices"
        ...
        - Use `internal` scheme for VPC-only access to keep Postgres1st WatchTower private
...
=== "On-Premise (MetalLB)"
    ...
    **Configure and deploy Postgres1st WatchTower**
    {.power-number}
...
=== "NodePort"
    ...
    **Best for**: Testing environments or when LoadBalancer is unavailable
    ...
    NodePort exposes Postgres1st WatchTower on a static port on each cluster node.
    {.power-number}
...
Postgres1st WatchTower ships with self-signed SSL certificates. For production, provide your own certificates:
...
Postgres1st WatchTower HA stores data in distributed databases, not on the PMM server pods themselves. To increase storage capacity, configure the ClickHouse and VictoriaMetrics clusters.
...
Postgres1st WatchTower HA uses environment variables to control its behavior. The HA-specific variables are pre-configured for optimal cluster operation, while data retention and other settings can be customized to match your requirements.
...
These variables are tested and validated for the HA architecture - modifying them is not recommended. Postgres1st WatchTower updates are managed through Helm chart upgrades rather than the UI to ensure consistency across all replicas.
...
- **Additional variables**: See [Postgres1st WatchTower environment variables documentation](../install-pmm/install-pmm-server/deployment-options/docker/env_var.md) for all available options.
...
| `secret.name` | Name of the Postgres1st WatchTower secret | `pmm-secret` |
...
| `pg-db.pmm.enabled` | Enable automatic Postgres1st WatchTower monitoring of PostgreSQL | `true` |
...
## Use and maintain Postgres1st WatchTower HA
...
To add PostgreSQL monitoring to Postgres1st WatchTower HA, see [Deploy a PMM client](https://docs.percona.com/percona-operator-for-postgresql/latest/monitoring.html#deploy-a-pmm-client) in the Percona PostgreSQL Operator documentation.
...
To create additional service tokens manually, see the [Postgres1st WatchTower documentation on service accounts](https://docs.percona.com/percona-monitoring-and-management/api/authentication.html).

### Monitor HA features

#### Monitor cluster health

Use the **Postgres1st WatchTower HA Health Overview** dashboard to monitor your entire HA deployment from a single view. 
...
Access the dashboard from **All Dashboards > Browse all dashboards > Experimental > Postgres1st WatchTower HA Health Overview**.
...
For detailed information about each panel and what to check, see the [Postgres1st WatchTower HA Health Overview dashboard reference](../reference/dashboards/dashboard-ha-health-overview.md).

#### Identify the leader node

Postgres1st WatchTower displays a visual badge on the side menu and displays the name of the active Postgres1st WatchTower instance that's currently handling all monitoring operations. For example, `pmm-ha-0`, `pmm-ha-1`, or `pmm-ha-2`.

Check this to quickly identify which server is active without needing to query the cluster directly:

![Postgres1st WatchTower HA Status badge](../images/HA-Status.png)
...
View detailed role and health information for all Postgres1st WatchTower nodes in one place.
{.power-number}
...
When you scale Postgres1st WatchTower HA up or down, **all Postgres1st WatchTower pods will be recreated**. This happens because the `PMM_HA_PEERS` environment variable is dynamically generated based on replica count and must be updated on all pods.
...
Postgres1st WatchTower images can be large (several GB). Before performing upgrades or scaling operations, pre-pull images on all nodes to avoid timeout issues:
...
To check the health of your Postgres1st WatchTower HA deployment:

```sh
# Check all Postgres1st WatchTower HA resources
...
For programmatic access to cluster status and node information, Postgres1st WatchTower HA provides REST API endpoints. These endpoints let you integrate HA monitoring into automation scripts, monitoring dashboards, or alerting systems.

Available endpoints:

- `GET /v1/ha/status`: Check if Postgres1st WatchTower is running in HA mode
...
### Modify your Postgres1st WatchTower HA deployment

This Tech Preview does not support upgrading between Postgres1st WatchTower versions. You can only modify configuration within the same version.

Use Helm upgrades to modify settings like resource limits, replica counts, or storage sizes within your current Postgres1st WatchTower version. Rolling updates ensure zero downtime. Each pod updates sequentially while HAProxy keeps traffic flowing to healthy nodes.
...
**Issue**: Postgres1st WatchTower not accessible after install
...
|-------|--------|------------|
| **[PMM-14704](https://perconadev.atlassian.net/browse/PMM-14704)**: PostgreSQL nodes in dropdown | Node selector shows database instances alongside Postgres1st WatchTower nodes | Select only nodes named `pmm-ha-0`, `pmm-ha-1`, `pmm-ha-2` |
| **[PMM-14705](https://perconadev.atlassian.net/browse/PMM-14705)**: CLI-added services show no metrics | Services from `pfw-admin` appear as UNSPECIFIED, dashboards empty (QAN works) | Add services via Postgres1st WatchTower UI instead |
...
| **[PMM-14734](https://perconadev.atlassian.net/browse/PMM-14734)**: Incorrect status | HA badge on Postgres1st WatchTower Home Dashboard may not reflect true cluster health | Use Inventory view or kubectl commands to check actual cluster status |                                   |
...
When scaling down to a single Postgres1st WatchTower replica (from 3 to 1), ensure the **Raft leader is on pmm-0** before scaling. Kubernetes StatefulSets remove pods in reverse ordinal order (highest first).
    
  - Scaling 3→1 removes pmm-2 and pmm-1, keeping only pmm-0
  - **If the Raft leader is on pmm-1 or pmm-2 when you scale down, Postgres1st WatchTower will become unreachable**
...
Postgres1st WatchTower HA Tech Preview does not support these VictoriaMetrics Enterprise features:

- **Prometheus data file reading**: Cannot import existing Prometheus data files into Postgres1st WatchTower HA
...
## Uninstall Postgres1st WatchTower HA

When uninstalling Postgres1st WatchTower HA, make sure to follow this exact order.    Uninstalling out of sequence leaves orphaned resources that cannot be auto-cleaned.

### Step 1: Remove Postgres1st WatchTower HA deployment
...
# WARNING: This deletes ALL PVCs in the pmm namespace, not just Postgres1st WatchTower HA
...
```sh
# Check for remaining Postgres1st WatchTower resources
