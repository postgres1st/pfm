...
!!! warning alert alert-success "Warning"
    At the moment of creation of this advisor, the Postgres1st WatchTower advisor engine connects only to the *pmm* database, so this advisor only checks that database. We would need the engine to be able to establish connections to each database in the PostgreSQL instance to get information.
