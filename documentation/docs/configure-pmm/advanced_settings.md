# Advanced Postgres1st WatchTower settings
...
The **Telemetry** switch enables gathering and sending basic **anonymous** data to Percona, which helps us to determine where to focus the development and what is the uptake for each release of Postgres1st WatchTower. 
...
Postgres1st WatchTower Telemetry is based on data collected by various Postgres1st WatchTower components and stored inside PMM Server 

!!! note alert alert-primary ""
    When Postgres1st WatchTower is installed, telemetry is not sent immediately. Before the first telemetry report is generated, Postgres1st WatchTower provides users with a 24-hour grace period to disable telemetry.
...
Grafana’s [anonymous usage statistics](https://grafana.com/docs/grafana/latest/administration/configuration/#reporting-enabled) is not managed by Postgres1st WatchTower. To activate it, you must change the PMM Server container configuration after each update.
...
When active, Postgres1st WatchTower will automatically check for updates and put a notification in the home page **Updates** dashboard if any are available.

## Advisors

Advisors are sets of checks grouped by functionality that run a range of database health checks on a registered Postgres1st WatchTower instance.
...
By default, Query Analytics (QAN) does not display queries from PMM Server's internal PostgreSQL database, keeping your dashboards focused on monitored databases rather than Postgres1st WatchTower's operational activity.
...
You'll see queries related to Postgres1st WatchTower's inventory, settings, advisor checks, alerts, backups, and authentication. These queries are typically lightweight so any spikes in volume, latency, or unexpected entries may indicate performance issues or database misuse.

!!! warning
    The default `postgres` database should only be used by Postgres1st WatchTower's internal operations. Applications should always use dedicated databases.
