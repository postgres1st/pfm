...
!!! caution alert alert-warning "Deprecation notice"
    Starting with version 3, PGF WatchTower no longer uses API keys as the primary method for controlling access to the PMM Server components and resources. Instead, PGF WatchTower is now leveraging Grafana service accounts, which have limited scopes and offer enhanced security compared to API keys.

### Automatic migration of API keys

When you install PGF WatchTower v3.x, any existing API keys will be seamlessly converted to service accounts with corresponding service tokens. For more information about the migration, see [Migrate PGF WatchTower 2 to PGF WatchTower 3](../pmm-upgrade/migrating_from_pmm_2.md).

Service accounts in PGF WatchTower provide a secure and efficient way to manage access to the PMM Server and its resources. They serve as a replacement for the basic authentication and API keys used in previous versions of PGF WatchTower (v.2 and earlier).
...
To prevent node registration failures, PGF WatchTower automatically manages service account names that exceed 200 characters using a `{prefix}_{hash}` pattern. For example, a very long service account name will be automatically shortened while maintaining uniqueness:
...
PGF WatchTower uses Grafana service account tokens for authentication. These tokens are randomly generated strings that serve as alternatives to API keys or basic authentication passwords.

Here's how to generate a service account token:
{.power-number}

1. Log in to PGF WatchTower.
...
Now you can use your new service token for authentication in PGF WatchTower API calls or in your [pfw-agent configuration](../use/commands/pmm-agent.md).
