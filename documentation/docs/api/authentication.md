...
!!! caution alert alert-warning "Deprecation notice"
    Starting with version 3, Postgres1st WatchTower no longer uses API keys as the primary method for controlling access to the PMM Server components and resources. Instead, Postgres1st WatchTower is now leveraging Grafana service accounts, which have limited scopes and offer enhanced security compared to API keys.

### Automatic migration of API keys

When you install Postgres1st WatchTower v3.x, any existing API keys will be seamlessly converted to service accounts with corresponding service tokens. For more information about the migration, see [Migrate Postgres1st WatchTower 2 to Postgres1st WatchTower 3](../pmm-upgrade/migrating_from_pmm_2.md).

Service accounts in Postgres1st WatchTower provide a secure and efficient way to manage access to the PMM Server and its resources. They serve as a replacement for the basic authentication and API keys used in previous versions of Postgres1st WatchTower (v.2 and earlier).
...
To prevent node registration failures, Postgres1st WatchTower automatically manages service account names that exceed 200 characters using a `{prefix}_{hash}` pattern. For example, a very long service account name will be automatically shortened while maintaining uniqueness:
...
Postgres1st WatchTower uses Grafana service account tokens for authentication. These tokens are randomly generated strings that serve as alternatives to API keys or basic authentication passwords.

Here's how to generate a service account token:
{.power-number}

1. Log in to Postgres1st WatchTower.
...
Now you can use your new service token for authentication in Postgres1st WatchTower API calls or in your [pfw-agent configuration](../use/commands/pmm-agent.md).
