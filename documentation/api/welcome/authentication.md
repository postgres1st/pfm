...
Bearer authentication uses security tokens called bearer tokens. In Postgres1st WatchTower 3, these are service account tokens generated by the server admin from the Settings UI or via the API. To learn how to generate a service account token, see [Authentication with service accounts](https://docs.percona.com/percona-monitoring-and-management/3/api/authentication.html#generate-a-service-account-and-token). Include the token in the `Authorization` header when making requests to protected resources:
